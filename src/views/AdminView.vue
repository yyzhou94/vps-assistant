      ...
      <main class="admin-main">
        ...
        <div v-if="activeTab === 'categories'" class="tab-content">
          <CategoryManager
            ...
            @script-clicked="handleScriptClicked"
            :loading="saving"
          />
        </div>
...
const navTitle = ref('与光同程') // 保存网站标题
const selectedCategoryId = ref('') // 用于站点管理的选中分类
const highlightedSiteId = ref('') // 新增：用于传递高亮ID
...
const loadCategories = async () => {
  ...
  try {
    ...
    navTitle.value = mockData.title || '与光同程'
...
// 修改：处理从 CategoryManager 发出的脚本点击事件
const handleScriptClicked = ({ siteId, categoryId }) => {
  // 1. 切换Tab并设置分类
  switchToSiteManager(categoryId)

  // 2. 立即清除上一个高亮ID，以防万一
  highlightedSiteId.value = ''

  // 3. 使用 setTimeout 将设置高亮ID的操作推迟到下一个宏任务
  //    这给了 SiteManager 组件充分的渲染和布局时间
  setTimeout(() => {
    // 4. 现在再设置高亮ID，触发 SiteManager 中的滚动逻辑
    highlightedSiteId.value = siteId

    // 5. (可选但推荐) 1.5秒后清除高亮，以便下次还能触发
    setTimeout(() => {
      highlightedSiteId.value = ''
    }, 1500)
  }, 50) // 50ms的延迟对于确保渲染完成通常是足够的
}
